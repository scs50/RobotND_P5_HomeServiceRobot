...
private:
  ...
  double start_goal[2] = {-2.0, -3.3};
  double end_goal[2] = {-8.0, 0.5};
...
public:
  ...
  void odomCallback(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    float x_error	    = fabs(x_pos - start_goal[0]);
    float y_error	    = fabs(y_pos - start_goal[1]);
    float tolerance 	    = 0.5;
    ...
    ROS_INFO("start_goal_x: [%f]", start_goal[0]);
    ROS_INFO("start_goal_y: [%f]", start_goal[1]);
